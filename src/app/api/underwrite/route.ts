...
import { EVALUATION_POLICY_VERSION, evaluateAssetPolicy } from "@/lib/evaluation-policy";
...
function blockedReport(gate: Gate, decision: "manual_review" | "rejected", forcedCode?: UnderwritingRejectionCode): UnderwritingReport {
  ...
  const policy = evaluateAssetPolicy({ tangible: gate.isTangibleAsset, evidenceScore: gate.assetEvidenceScore, authenticityScore: gate.authenticityScore, confidenceScore: 0, riskScore: 100, valuationUsd: 0, fatalFlags: gate.fatalFlags });
  return { asset: { name: "Asset evidence unavailable", category: gate.sourceType, brand: "Unknown", model: "Unknown", condition: "Unknown", identifier: "Not provided", ownershipClaim: "Self-attested; not verified" }, sourceType: gate.sourceType, assetEvidenceScore: gate.assetEvidenceScore, authenticityScore: gate.authenticityScore, valuationConfidence: 0, valuationUsd: 0, launchValuationUsd: 0, valuationRange: [0, 0], riskScore: policy.normalizedRiskScore, riskFlags: gate.fatalFlags.length ? gate.fatalFlags : ["Insufficient trustworthy asset evidence"], evidenceFound: gate.evidenceFound, missingEvidence: gate.missingEvidence, decision, decisionReasons: policy.reasons.length ? policy.reasons : [gate.reasoning || "The upload did not meet the evidence threshold."], policyVersion: EVALUATION_POLICY_VERSION, policyChecks: policy.checks, summary: decision === "rejected" ? "This upload cannot be minted yet because the protocol could not establish a recognizable original physical asset photo." : "This upload needs a clearer original photo before tokenization.", mintEligible: false, ownershipVerified: false, rejection: { code, message: decision === "manual_review" ? "A clearer original photo is needed before this asset can be minted." : "This upload was not accepted as trustworthy physical-asset evidence.", suggestions, retryable: true, requiresFreshPhoto: true } };
}
function normalizeReport(raw: Record<string, unknown>, gate: Gate): UnderwritingReport {
  ...
  const policy = evaluateAssetPolicy({ tangible: gate.isTangibleAsset, evidenceScore: gate.assetEvidenceScore, authenticityScore: gate.authenticityScore, confidenceScore: confidence, riskScore: Number(raw.riskScore ?? 50), valuationUsd: point, fatalFlags: gate.fatalFlags });
  return { asset: { name: String(asset.name ?? "Physical asset"), category: String(asset.category ?? gate.sourceType), brand: String(asset.brand ?? "Unknown"), model: String(asset.model ?? "Unknown"), condition: String(asset.condition ?? "Not fully established"), identifier: String(asset.identifier ?? "Not provided"), ownershipClaim: "Self-attested; not verified" }, sourceType: gate.sourceType, assetEvidenceScore: gate.assetEvidenceScore, authenticityScore: gate.authenticityScore, valuationConfidence: confidence, valuationUsd: point, launchValuationUsd: Math.min(point, low), valuationRange: [low, high], riskScore: policy.normalizedRiskScore, riskFlags: strings(raw.riskFlags), evidenceFound: gate.evidenceFound, missingEvidence: gate.missingEvidence, decision: policy.approved ? "approved" : "manual_review", decisionReasons: policy.approved ? ["The upload passed the deterministic asset-evidence policy."] : policy.reasons, policyVersion: EVALUATION_POLICY_VERSION, policyChecks: policy.checks, summary: String(raw.summary ?? "Conservative second-hand valuation based on supplied evidence."), mintEligible: policy.approved, ownershipVerified: false };
}
function offlineResponse(file: File, text: string): UnderwritingResponse {
  const isImage = file.type.startsWith("image/");
  const report: UnderwritingReport = { asset: { name: file.name.replace(/\.[^.]+$/, "") || "Uploaded physical asset", category: isImage ? "Physical asset photo" : "Asset record", brand: "Unknown", model: "Unknown", condition: "Needs live AI inspection", identifier: "Not provided", ownershipClaim: "Self-attested; not verified" }, sourceType: file.type || "upload", assetEvidenceScore: isImage ? 58 : text.length > 80 ? 45 : 20, authenticityScore: isImage ? 50 : 35, valuationConfidence: 0, valuationUsd: 0, launchValuationUsd: 0, valuationRange: [0, 0], riskScore: 100, riskFlags: ["Live AI evaluator unavailable"], evidenceFound: [file.name], missingEvidence: ["Live visual identification", "Conservative market valuation"], decision: "manual_review", decisionReasons: ["Gemini is unavailable, so deterministic policy refuses to invent a valuation."], policyVersion: EVALUATION_POLICY_VERSION, summary: "The upload was received, but live AI underwriting is required before mint authorization.", mintEligible: false, ownershipVerified: false, mock: true, fallbackReason: "Live evaluation is unavailable. Try again shortly.", rejection: { code: "MANUAL_REVIEW", message: "Live AI underwriting is required before this asset can be minted.", suggestions: ["Try the evaluation again shortly.", "Use one clear original photo of the physical asset."], retryable: true, requiresFreshPhoto: isImage } };
  return { report };
}
