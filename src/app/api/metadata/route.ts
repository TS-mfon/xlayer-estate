...
import { assertEvaluationChain, hashText, signMintAuthorization, verifyEvaluationToken, verifyImageToken } from "@/lib/attestation";
import { serializeReport } from "@/lib/metadata";
import { storeJson } from "@/lib/github-storage";
import { TOTAL_SHARES } from "@/lib/config";
import { configuredNetwork, isSupportedChainId, type SupportedChainId } from "@/lib/network";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const chainId = Number(form.get("chainId") ?? "1952") as SupportedChainId;
    ...
    const claims = verifyEvaluationToken(reportHash, evaluation);
    assertEvaluationChain(claims, chainId);
    verifyImageToken(reportHash, image, imageToken, chainId);
    ...
    const signed = await signMintAuthorization({ chainId, to: recipient, valuationUsd: BigInt(Math.round(report.valuationUsd)), launchValuationUsd: BigInt(Math.round(report.launchValuationUsd)), riskScore: report.riskScore, underwritingHash: reportHash, metadataHash, totalShares: TOTAL_SHARES, nonce, deadline });
    return NextResponse.json({ hash: reportHash, metadataHash, uri: metadataStorage.uri, imageUri: image.uri, pinned: metadataStorage.storage === "github", storage: metadataStorage.storage, nonce: nonce.toString(), deadline: deadline.toString(), signature: signed.signature, underwriter: signed.underwriter, contract: network.registry, chainId });
  } catch (error) {
