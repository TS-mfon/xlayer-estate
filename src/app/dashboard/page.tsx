...
async function loadWalletAssets(wallet: string, chainId: number) {
  const response = await fetch(`/api/assets?wallet=${encodeURIComponent(wallet)}&chainId=${chainId}`, { cache: "no-store" });
...
export default function DashboardPage() {
  ...
  const portfolio = useQuery({ queryKey: ["wallet-estate", network.id, address], enabled: Boolean(address), retry: 1, staleTime: 15_000, queryFn: () => loadWalletAssets(address!, network.id) });
  ...
  return <div className="page-frame estate-dashboard">
    ...
    <EstateSection title="Live properties" subtitle={`Assets with active ${network.settlementLabel} markets and live share prices.`} assets={listed} address={address} />
