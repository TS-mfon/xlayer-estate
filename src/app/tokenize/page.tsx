...
export default function TokenizePage() {
  ...
  const onFile = async (file: File) => {
    setPhase("underwriting"); setError(""); setReport(null); setEvaluationToken(""); setTxHash(undefined); setTokenId(""); setImage(file.type.startsWith("image/") ? file : null);
    try {
  ...
  return <div className="page-frame">
    <div className="section-heading"><div><p className="kicker">Asset origination / 1952</p><h1 className="text-5xl font-semibold tracking-[-.06em] sm:text-7xl">Tokenize<br />anything physical.</h1></div><p>Upload a clear photo or record. AI estimates resale value and risk without requiring confidential ownership documents.</p></div>
    <div className="mb-7 flex flex-wrap gap-2 text-xs text-white/45">{["Asset", "AI gate", "Mint", "Listed"].map((label, i) => <span key={label} className={`rounded-full border px-3 py-1.5 ${step > i ? "border-cyan-300/40 bg-cyan-300/10 text-cyan-100" : "border-white/10"}`}>{String(i + 1).padStart(2, "0")} / {label}</span>)}</div>
    ...
    {report && <div className="space-y-4"><UnderwritingReport report={report} />{report.decision === "approved" && <div className="glass-panel flex flex-wrap items-center gap-3 p-4 text-sm"><span className="text-white/60">{image ? `NFT image: ${image.name}` : "A branded asset card will be generated."}</span><span className="text-white/40">Ownership is self-attested and never legally verified by this demo.</span></div>}<div className="flex flex-wrap items-center gap-3"><button className="button button-primary" disabled={!canMint || phase === "minting"} onClick={onMint}>{phase === "minting" || txPending ? "Writing asset token…" : report.decision === "approved" ? "Mint tradable asset ↗" : "Mint unavailable"}</button><button className="button button-ghost" onClick={() => { setReport(null); setPhase("idle"); }}>Start over</button>{txHash && <a className="text-sm text-cyan-200" href={explorerTx(xlayerTestnet.id, txHash)} target="_blank" rel="noreferrer">View transaction ↗</a>}</div>{phase === "done" && <div className="glass-panel p-5 text-sm text-emerald-200">Asset #{tokenId || "confirmed"} is minted. {metadataPinned ? "Metadata and image are pinned to IPFS." : "Demo metadata uses a content-addressed fallback."} {tokenId && <a className="ml-2 underline" href={`/marketplace/${tokenId}`}>Open marketplace ↗</a>}</div>}{phase === "error" && <div className="glass-panel p-4 text-sm text-red-200">{error}</div>}{report.decision !== "approved" && <div className="glass-panel p-4 text-sm text-amber-100">This upload cannot be minted until the asset passes the evidence and authenticity thresholds.</div>}</div>}
