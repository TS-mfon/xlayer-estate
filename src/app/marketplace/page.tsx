...
async function loadMarkets(chainId: number) {
  const response = await fetch(`/api/markets?chainId=${chainId}`, { cache: "no-store" });
...
export default function MarketplacePage() {
  ...
  const markets = useQuery({ queryKey: ["marketplace", network.id], queryFn: () => loadMarkets(network.id), staleTime: 15_000, refetchInterval: 15_000 });
  ...
  return <div className="page-frame marketplace-world marketplace-simple">
    <div className="marketplace-simple-heading"><div><p className="kicker">Listed assets / {network.settlementLabel}</p><h1>Trade live<br/>asset shares.</h1></div><p>Only assets with an active liquidity pool on {network.shortLabel.toLowerCase()} appear here. Prices are read from current X Layer reserves.</p></div>
    ...
    <div className="estate-grid">{visibleMarkets.map((market) => <AssetCard key={market.id} id={BigInt(market.id)} info={deserializeAssetInfo(market)} chainId={network.id} listed pricing={market.pricing ?? undefined} />)}</div>
  </div>;
}
