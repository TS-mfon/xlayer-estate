...
export function RouteHero({ eyebrow, title, description, aside, actions }: { eyebrow: string; title: ReactNode; description: string; aside?: ReactNode; actions?: ReactNode }) {
  return <motion.header className="route-hero" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .75, ease: [.22, 1, .36, 1] }}>
    ...
    <div className="route-hero-visual"><div className="route-hero-monolith"><span className="monolith-core"><img src="/icon.svg" alt="" /></span><span className="monolith-ring ring-one"/><span className="monolith-ring ring-two"/><span className="monolith-label">ASSET PASSPORT PROTOCOL</span></div>{aside && <div className="route-hero-aside">{aside}</div>}</div>
  </motion.header>;
}
