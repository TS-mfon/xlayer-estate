...
export function UploadDropzone({ onFile, onImage, loading }: { onFile: (file: File) => void; onImage?: (file: File) => void; loading?: boolean }) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label="Upload an original physical asset photo"
    ...
    >
      <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => handle(event.target.files)} />
