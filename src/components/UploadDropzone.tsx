...
export function UploadDropzone({ onFile, loading }: { onFile: (file: File) => void; loading?: boolean }) {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept="image/*,.pdf,.txt,.md,.csv,.json" className="hidden" onChange={(event) => handle(event.target.files)} />
      ...
      <div className="relative">
        ...
        <h2 className="mt-3 text-2xl font-semibold tracking-tight">{loading ? "Extracting the evidence…" : "Drop property evidence here"}</h2>
        <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-white/45">Deed, title, valuation, listing, PDF, image, or text. Maximum file size 12 MB.</p>
      </div>
    </div>
  );
}
