...
export function FaucetButton({ onMinted }: { onMinted?: () => void }) {
  ...
  async function mint() {
    ...
    try { if (!client) throw new Error("X Layer RPC is not ready"); const request = { address: USDC_ADDRESS, abi: erc20Abi, functionName: "mint", args: [address, parseUnits("25", 6)] } as const; await simulateContractWrite(client, address, request as unknown as Record<string, unknown>); const hash = await writeContractAsync({ ...request, chainId: xlayerTestnet.id }); assertSuccessfulReceipt(await client.waitForTransactionReceipt({ hash })); setState("done"); onMinted?.(); }
