...
export type AssetInfo = { owner: string; valuationUsd: bigint; launchValuationUsd?: bigint; totalShares?: bigint; riskScore: number; status: number; underwritingHash: string; metadataHash?: string; metadataURI: string; timestamp: bigint };
...
export function AssetCard({ id, info, chainId, balance, listed, wallet }: { id: bigint; info: AssetInfo; chainId: number; balance?: bigint; listed?: boolean; wallet?: string }) {
  ...
  return <article className="estate-card group">
    ...
    <div className="estate-card-body"><div><p className="kicker">{metadata?.attributes.find((item) => item.trait_type === "Asset category")?.value ?? "Digital property"}</p><h2>{metadata?.name ?? `Asset #${id}`}</h2><p className="estate-card-owner">Issuer {shortAddress(info.owner)} {issuer ? "· you" : ""}</p></div>
      <div className="estate-card-stats"><Stat label="AI value" value={formatUsd(info.valuationUsd)} /><Stat label="Launch" value={formatUsd(info.launchValuationUsd ?? info.valuationUsd)} /><Stat label="Risk" value={`${info.riskScore}/100`} color={risk.color} />{balance !== undefined && <Stat label="Your shares" value={balance.toLocaleString()} />}</div>
