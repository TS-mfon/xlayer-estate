...
interface StoredObject { uri: string; storage: "github" | "data"; sha256: string }
...
export async function storeBytes(path: string, bytes: Buffer, contentType: string): Promise<StoredObject> {
  ...
  if (!commitSha) {
    ...
    const commits = await fetch(
      `https://api.github.com/repos/${github.repo}/commits?path=${encodeURIComponent(path)}&sha=${encodeURIComponent(github.branch)}&per_page=1`,
