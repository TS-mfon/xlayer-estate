export function friendlyError(error: unknown, fallback = "Something went wrong") {
  ...
  if (/transfer amount exceeds balance|exceeds balance/i.test(message)) return "Your USDC_TEST balance is below the seed plus protocol fee required for this action.";
  ...
  if (/seed below|seed too small/i.test(message)) return "The liquidity seed must be at least 10.00 USDC_TEST.";
  ...
  if (/trade below fee/i.test(message)) return "The trade must be greater than the fixed 0.20 USDC_TEST fee.";
